...
Compose 或编排系统中的中继，再通过 `C2_RELAY_ANCHOR_ADDRESS` 或
`cc.set_relay_anchor()` 让 Python 代码连接它的 relay anchor。anchor 只负责
注册和按名称解析；远程 HTTP 调用仍会直连解析得到的 `relay_url`，只有
anchor 是 loopback/local 端点时才会选择本地 direct IPC。
