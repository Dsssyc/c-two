...
| --- | --- | --- |
| Resource-first contract descriptor, codec refs, provider resolution, and codegen foundation | Python now has `CodecRef`, `@cc.transferable(codec_ref=...)`, `cc.use_codec(...)`, `cc.bind_codec(...)`, resource/CRM conformance checks, portable export fail-fast for pickle-only wire refs, Rust `c2-contract` validation for `c-two.contract.v1`, Rust `c2-codegen` TypeScript skeleton generation, `c3 contract validate/export/infer/codegen typescript`, `cc.infer_crm_from_resource(...)`, `c-two.control.json.v1` for JSON-safe primitive/control parameters and returns, grid Arrow IPC payloads expressed as codec refs, optional fastdb provider smoke coverage, and fastdb-owned TypeScript helper stub generation from `fastdb.schema.v1`. Remaining gaps are reusable generic py-arrow provider packaging, reusable fastdb provider packaging, and wiring real TypeScript/WASM payload codec runtimes behind the generated helper stubs. | Keep C-Two core codec-neutral. Keep canonical portable descriptor validation in Rust `c2-contract`, keep primitive/control envelopes separate from large payload codecs, graduate example/provider pilots into documented provider packages, generate only honest SDK skeletons plus codec requirements until provider-specific runtimes exist, and keep pickle as Python-only fallback. |
