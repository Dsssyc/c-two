...
| --- | --- | --- |
| Resource-first contract descriptor, codec refs, provider resolution, and codegen foundation | Python now has `CodecRef`, `@cc.transferable(codec_ref=...)`, `cc.use_codec(...)`, `cc.bind_codec(...)`, resource/CRM conformance checks, portable export fail-fast for pickle-only wire refs, Rust `c2-contract` validation for `c-two.contract.v1`, `c3 contract validate`, `c-two.control.json.v1` for JSON-safe primitive/control parameters and returns, grid Arrow IPC payloads expressed as codec refs, and optional fastdb provider smoke coverage. Remaining gaps are `c3 contract export/infer`, reusable generic py-arrow and fastdb provider packaging, resource-to-CRM infer, SDK codegen, and provider-owned fastdb codegen helpers. | Keep C-Two core codec-neutral. Keep canonical portable descriptor validation in Rust `c2-contract`, keep primitive/control envelopes separate from large payload codecs, graduate example/provider pilots into documented provider packages, generate only honest SDK skeletons plus codec requirements until provider-specific helpers exist, and keep pickle as Python-only fallback. |
