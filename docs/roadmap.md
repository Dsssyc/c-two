...
| --- | --- | --- |
| Resource-first contract descriptor, codec refs, and provider resolution | Python now has `CodecRef`, `@cc.transferable(codec_ref=...)`, `cc.use_codec(...)`, `cc.bind_codec(...)`, resource/CRM conformance checks, portable export fail-fast for pickle-only wire refs, grid Arrow IPC payloads expressed as codec refs, and optional fastdb provider smoke coverage. Remaining gaps are Rust `c2-contract` validation for full `c-two.contract.v1`, primitive/control portable envelopes, reusable generic py-arrow and fastdb provider packaging, resource-to-CRM infer, `c3 contract infer/export`, and SDK codegen. | Keep C-Two core codec-neutral. Move canonical portable descriptor validation into Rust `c2-contract`, define portable primitive/control envelopes, graduate example/provider pilots into documented provider packages, and keep pickle as Python-only fallback. |
