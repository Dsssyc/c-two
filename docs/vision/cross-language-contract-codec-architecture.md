...
本文档是当前方向和阶段计划，不表示所有示例 API 都已经完整可用。今天已经存在的基础包括 Python CRM、`@cc.transferable(abi_id=..., abi_schema=...)`、`@cc.transferable(codec_ref=...)`、`CodecRef`、`cc.use_codec(...)`、`cc.bind_codec(...)`、provider 在单参数/返回值场景下优先于 pickle fallback 的解析、resource/CRM conformance check、`build_contract_descriptor(..., portable=True)` 对 pickle-only wire ref 的拒绝、`cc.export_contract_descriptor(...)`、`python -m c_two.cli.contract export module:CRM`、descriptor hashing、custom ABI 引用、Python-only pickle fallback、direct IPC、relay、grid 示例中显式 `CodecRef` 化的 Arrow IPC payload，以及 fastdb 侧可选 C-Two wrapper provider 冒烟。尚未实现的部分包括 Rust `c2-contract` 对 `c-two.contract.v1` 的完整 portable descriptor 校验、resource-to-CRM infer、`c3 contract infer/export` 命令、通用 py-arrow provider、primitive/control envelope portable codec，以及完整 SDK codegen。
...
py-arrow 是第一个 mature external format provider pilot。C-Two examples 已经用 Arrow IPC transferable 表达 grid payload，因此 py-arrow provider 可以从现有例子演进：第一步把手写 `@cc.transferable(abi_schema=...)` 收敛为 Arrow IPC `CodecRef` 和 adapter；下一步再把这类 adapter 抽成可复用 provider。需要注意，grid 的控制参数仍然是普通 Python 类型，在 primitive/control envelope 落地前不能把整个 Grid 合约视为 portable。
