...
本文档是当前方向和阶段计划，不表示所有示例 API 都已经完整可用。今天已经存在的基础包括 Python CRM、`@cc.transferable(abi_id=..., abi_schema=...)`、`@cc.transferable(codec_ref=...)`、`CodecRef`、`cc.use_codec(...)`、`cc.bind_codec(...)`、provider 在单参数/返回值场景下优先于 pickle fallback 的解析、resource/CRM conformance check、`build_contract_descriptor(..., portable=True)` 对 pickle-only wire ref 的拒绝、`cc.export_contract_descriptor(...)`、`cc.infer_crm_from_resource(...)`、`python -m c_two.cli.contract export/infer module:Class`、Rust `c2-contract` 对 `c-two.contract.v1` 的 portable descriptor 结构校验、`c3 contract validate/export/infer`、`c-two.control.json.v1` primitive/control envelope、descriptor hashing、custom ABI 引用、Python-only pickle fallback、direct IPC、relay、grid 示例中显式 `CodecRef` 化的 Arrow IPC payload，以及 fastdb 侧可选 C-Two wrapper provider 冒烟。当前必须继续补齐的部分包括通用 py-arrow provider 包装、SDK codegen，以及 fastdb 侧基于 `fastdb.schema.v1` 的 provider codegen integration。
...
py-arrow 是第一个 mature external format provider pilot。C-Two examples 已经把 grid Arrow IPC payload 收敛为显式 Arrow IPC `CodecRef` 和 adapter，控制参数也已经能通过 `c-two.control.json.v1` 脱离 pickle；下一步是把这类 adapter 抽成可复用 provider，并补上 provider/codegen 自动组合能力。
...
11. 下一步：实现首个 SDK codegen 切片，优先 TypeScript，因为 fastdb 已有 TS 工具链；生成器只生成 contract skeleton、typed call surface、contract hash constants 和 codec requirement declarations，不伪造未知 codec 的实现。
