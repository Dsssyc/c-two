...
12. 下一步：在 fastdb 侧实现 provider-owned codegen helper，把 `fastdb.schema.v1` 生成 TypeScript payload codec helper 或明确的 unsupported stub，应用层组合 c-two RPC skeleton 与 fastdb payload helper。
