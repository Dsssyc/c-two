...
- `sdk/python/native/Cargo.lock`: commit the generated lockfile for reproducible Python native extension builds.
- `sdk/python/pyproject.toml`: set maturin `manifest-path = "native/Cargo.toml"`.
- `pyproject.toml` and `sdk/python/pyproject.toml`: include `sdk/python/native` files in uv cache keys.
...
- Create: `sdk/python/native/Cargo.lock`
...
- [x] **Step 2: Remove the Python binding crate from core workspace**
...
- [x] **Step 3: Update Python native crate metadata and paths**
...
- [x] **Step 4: Update the PyO3 module docs**
...
- [x] **Step 5: Update maturin manifest path**
...
- [x] **Step 6: Verify moved crate compiles as a Python native crate**
...
- [x] **Step 7: Update uv cache keys for the moved native crate**

In root `pyproject.toml`, add these entries to `[tool.uv].cache-keys`:

```toml
{ file = "sdk/python/native/Cargo.toml" },
{ file = "sdk/python/native/Cargo.lock" },
{ file = "sdk/python/native/**/*.rs" },
```

In `sdk/python/pyproject.toml`, add these entries to `[tool.uv].cache-keys`:

```toml
{ file = "native/Cargo.toml" },
{ file = "native/Cargo.lock" },
{ file = "native/**/*.rs" },
```

- [x] **Step 8: Verify Rust core workspace no longer sees PyO3 binding crate**
...
- [x] **Step 9: Run tests from Task 1 to verify GREEN**
...
- [x] **Step 10: Commit migration**

```bash
git add core/Cargo.toml pyproject.toml sdk/python/pyproject.toml sdk/python/native
...
- [x] **Step 1: Update core CI command**
...
- [x] **Step 2: Ensure workflow policy tests still include native path tests**
...
- [x] **Step 3: Update Python SDK README development section**
...
- [x] **Step 4: Update contributor docs**
...
- [x] **Step 6: Run workflow policy tests**
...
- [x] **Step 1: Update root README commands**
...
- [x] **Step 2: Keep VS Code linked projects language-neutral**
...
- [x] **Step 3: Search active files for old paths**
...
- [x] **Step 4: Commit active path updates**
...
- [x] **Step 1: Verify core Rust workspace**
...
- [x] **Step 2: Verify Python native extension build**
...
Result: `git diff --check` PASS. `cargo fmt --manifest-path core/Cargo.toml -- --check`
does not apply to the virtual `core/` workspace (`Failed to find targets`). The
workspace-equivalent command `cargo fmt --manifest-path core/Cargo.toml --all --
--check` reports pre-existing core rustfmt drift, and `cargo fmt --manifest-path
