...
| `seeds` | empty |
| `idle_timeout_secs` | `0` unless the team explicitly decides CLI operational default should be `300` |
...
`C2_RELAY_IDLE_TIMEOUT` is already tracked as a separate issue because CLI defaults to `300` while `RelayConfig::default()` and Python embedding default to disabled eviction. This spec recommends resolving that divergence by making `RelayConfig` canonical and requiring CLI to opt into any CLI-specific operational default explicitly if the team chooses one.
